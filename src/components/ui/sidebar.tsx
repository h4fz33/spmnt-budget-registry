...
type CssVariableStyle = React.CSSProperties & Record<`--${string}`, string | number | undefined>
type SidebarOpenChange = boolean | ((currentOpen: boolean) => boolean)

type SidebarContextValue = {
  state: "expanded" | "collapsed"
  open: boolean
  setOpen: (value: SidebarOpenChange) => void
  isMobile: boolean
  openMobile: boolean
...
type SidebarGroupLabelProps = Omit<
...
type SidebarGroupActionProps = Omit<
...
type SidebarMenuActionProps = Omit<
  useRender.ComponentProps<"button", { slot: string; sidebar: string }>,
  "className"
> & {
  className?: string
...
type SidebarMenuSubButtonProps = Omit<
  useRender.ComponentProps<"a", { slot: string; sidebar: string; size: "sm" | "md"; active: boolean }>,
  "className"
> & {
  className?: string
  size?: "sm" | "md"
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback((value: SidebarOpenChange) => {
  ...
  const sidebarStyle: CssVariableStyle = {
...
function Sidebar({
  ...
  if (isMobile) {
    const mobileSidebarStyle: CssVariableStyle = {
...
type SidebarMenuButtonProps = Omit<
  useRender.ComponentProps<
    "button",
    { slot: string; sidebar: string; size: "default" | "sm" | "lg"; active: boolean }
  >,
  "className"
> &
...
function SidebarMenuButton({
  ...
  const tooltipTriggerRender: React.ComponentProps<typeof TooltipTrigger>["render"] =
    typeof render === "function"
      ? (props, _tooltipState) => render(props, renderState)
      : render
  const comp = useRender({
    ...
    render: !tooltip ? render : <TooltipTrigger render={tooltipTriggerRender} />,
    state: renderState,
  })
...
function SidebarMenuSkeleton({
  ...
  const skeletonStyle: CssVariableStyle = {
