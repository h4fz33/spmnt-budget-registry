...
The seed process must validate required values, UTF-8 decoding, and uniqueness of all three columns before writing anything. It then upserts by `moe_code`, requires `smis_code` to remain unique, reports differences instead of silently renaming an existing School, and assigns the parent ESAO from explicit seed configuration because the CSV does not contain an ESAO code.
