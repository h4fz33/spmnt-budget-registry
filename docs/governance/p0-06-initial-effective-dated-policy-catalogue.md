...
| `POL-FF-04` | `FF-04` Retainable Non-Budgetary Payment | Payment decreases the applicable fund; Payment Voucher is atomic; School Director approval under `AUTH-11`; Cashbook cross-check required. | P0-03 `FF-04`; `manual_2515.md:184-214,522-533,648-653,665-678`; `AUTH-11` | Approved activity/source evidence and Payment Voucher; no partial payment. | A full approved voucher posts registry, cash position, and one cross-check atomically. |
...
| `POL-FF-11` | `FF-14` Internal Money-Position Transfer | `AUTH-34`: Finance Officer initiates only approved subtype; same Fund, atomic/net-zero; Director approval/acknowledgement only when this rule/source requires it; no Cashbook. | P0-03 `FF-14`; `manual_2515.md:129-133,150-155,609-640`; `reseach/FF-14 Internal money transfer.md`; `AUTH-34` | Deposit/withdrawal slip, passbook, paying-agency evidence; cross-School, cross-Fund, partial, duplicate, or generic transfer denies. | THB 2,000 moves from cash to bank in one Fund; total Fund balance remains THB 2,000. |
...
- `AUTH-11` payment and remittance commands require active School Director approval where the Matrix row requires it; no substitute role may approve.
- `AUTH-12` Official Advance issue requires active School Director approval and prior-unsettled checks.
- `AUTH-34` is the only transfer command. It is not a generic privilege and cannot cross School or Fund boundaries.
- `AUTH-14` and `AUTH-15` remain `DEFERRED-AND-DENIED`; no temporary or substitute Director authority exists.
...
- Director-required commands fail closed without an active School Director; `AUTH-14/15` remain denied.
