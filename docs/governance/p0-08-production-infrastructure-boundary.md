...
| Production | One authoritative shared PostgreSQL write model, private from direct end-user access, reached only by authorized application/runtime and operational paths. Production data and secrets never flow down to development/test. | Logical model and D01-D10 decisions are recorded; D11 non-production selected-provider proof COMPLETE (2026-08-16); production enablement remains blocked by unconfigured service controls and the separate final authorization in `P5-12` |
...
| `P0-08-D11` | Obtain separately authorized non-production selected-provider capability/configuration proof for Prisma Postgres and, only after separately authorized setup, Google Cloud Storage. This does not execute or accept application migrations, financial behavior, School isolation, Audit Log/history preservation, schema-integrity restore, or a full RPO/RTO drill; those remain assigned to later implementation/verification tasks. | `BLOCKED`: Product Owner approved Google Secret Manager and bounded `P0-08-D11-01` execution on 2026-08-16, then amended the selected GCS class to `NEARLINE`, ratified the removal of only the observed public binding, and approved the controlled Prisma runner. The class now matches the observed bucket; the no-public-binding condition still requires a current redacted recheck and the runner is not yet available. No provider capability or acceptance exists; production financial posting remains disabled and final production authorization belongs to `P5-12`. Evidence: `docs/governance/p0-08-d11-non-production-preflight.md` |
...
| `P0-08-D11` | `A` accepts sufficient non-production selected-provider evidence for P0-08 decision/architecture readiness; final production authorization remains `P5-12` | `O` produces provider proof; `S` accepts security/isolation proof; `P` accepts only the synthetic/anonymized test-boundary and recovery-feasibility evidence | Redacted pass/fail evidence for isolated availability/configuration facts, controlled-secret-delivery/custody boundary, TLS/encryption, a minimal non-business PostgreSQL compatibility harness, private GCS boundary only when separately setup-authorized, and observed backup/restore feasibility. No application migration, financial behavior, School-isolation, Audit Log/history, schema-integrity restore, or full RPO/RTO drill result is accepted here | `BLOCKED -- private-boundary recheck and approved Prisma runner unavailable` | Product Owner approval/amendment in the current Codex task; `docs/governance/p0-08-d11-non-production-preflight.md` | 2026-08-16 |
...
| `P0-08-D11` | Product Owner approved Google Secret Manager and bounded non-production setup/proof on 2026-08-16. The exact Prisma target is `proj_cmspqhtsz2dti12f55eww2w0o`; no result for the earlier truncated identifier is accepted. Authenticated GCP access verified the approved project and the three supplied Secret Manager resource names have enabled-version metadata without payload retrieval. The bucket location is `ASIA-SOUTHEAST3`; its observed `NEARLINE` class now matches the Product Owner-amended selection. The Product Owner ratified the earlier removal of only the observed `allUsers` `roles/storage.objectViewer` binding; no other IAM member is recorded, and a current redacted absence recheck is still required. The Product Owner approved a dedicated non-production Cloud Run Job that may access only the Prisma API-token secret and emit redacted HTTP/project/region results; System Admin has not yet made it available. | Required non-production provider proof and `A`/`O`/`S`/`P` acceptance are absent. The former class-mismatch and remediation-authorization gates are resolved as decisions, but private-boundary verification and the controlled Prisma runner remain required before proof can resume. The supplied Prisma `ap-southeast-1` region identifier could not be verified against the selected Singapore location, so it is not accepted as D01 evidence. No encryption-at-rest, PostgreSQL compatibility, backup/PITR, restore, or provider capability is accepted. D11 cannot grant production authorization; application migrations, financial behavior, School isolation, Audit Log/history preservation, schema-integrity restore, and full RPO/RTO drills remain later work. | `BLOCKED -- PRIVATE-BOUNDARY RECHECK AND PRISMA RUNNER UNAVAILABLE` |

### Required Sign-Off

| Authority | Decision | Name/reference | Date | Conditions |
| --- | --- | --- | --- | --- |
| `A`: Private Business accountable Product/Infrastructure Owner | D01, D02, D04, D05, D07-D10 selected; D11 `OPEN` | `OWNER-STATEMENT-2026-08-14-D01`; `OWNER-STATEMENT-2026-08-14`; current Product Owner instruction in this Codex task (no personal name recorded); ADR-0015 | `2026-08-15` | D01-D10 selections do not approve provider capability claims or production. Accepted non-production D11 evidence can make P0-08 decision/architecture-ready only; `P5-12` is the final production-authorization gate |
| `P`: Private Business / Product Owner | D03 connectivity boundary, D04/D05 business targets, and D10 business validation/final acceptance selected; D11 acceptance `OPEN` | Current Product Owner instruction in this Codex task (no personal name recorded) | `2026-08-15` | Connected drafts require later authorization; financial writes remain blocked through recovery validation; D11 test-boundary acceptance cannot grant production approval |
...
secret-delivery mechanism. Redacted preflight verified only project/Secret
Manager metadata and the GCS location, not provider capabilities. The bucket
reports the Product Owner-amended `NEARLINE` selection. The Product Owner has
ratified the earlier removal of only the observed public `allUsers`
`roles/storage.objectViewer` binding; this task made no further IAM change or
restoration and records no other IAM member. A current redacted verification
that the binding remains absent is still required. Prisma proof remains
blocked because the approved controlled non-exposing authenticated API runner
for the exact project is not yet available. It must continue to prohibit production access, real School
financial/personal data, credentials in source/client/logs, public bucket/URL
exposure, offline business-data reads, queued writes, local persistence, and
synchronization. No Prisma URI or GCS service-account JSON key has been read,
created, or stored by this task.

**Resumed execution condition - 2026-08-16:** The current sandbox has Google
Cloud SDK installed and is configured for `spmnt-sch-acc-audit`, but it cannot
access `gcloud`'s credential database (`WinError 5`) to list or invoke Cloud
Run Jobs. Runner availability is therefore unverified here; no account
identity, credential, job listing, or runner result was read. The approved
runner must be run from a controlled environment that can access its own
credential store and return only the redacted D11 result.
...
`defaultRegion=ap-southeast-1`. This is accepted as partial project metadata
only. The response has no HTTP status, runner/job attribution, or provider
source that maps the identifier to Singapore; its workspace and creation data
are not retained. It does not prove controlled-secret delivery, provider
configuration, or any D11 capability.
...
| Accountable approval makes production boundary enforceable | **Not satisfied; amended 2026-08-16** | D01-D10 decisions are recorded; the D11 class amendment, IAM-remediation ratification, and controlled-runner design are approved. A redacted no-`allUsers` recheck and a runner result remain absent, so the private-object boundary and Prisma proof cannot yet be accepted. Adequate D11 evidence can establish P0-08 decision/architecture readiness only; `P5-12` is required for production authorization |

**P0-08 status:** `BLOCKED`. D01-D10 decision values are recorded and the D11 class amendment, IAM-remediation ratification, and controlled-runner design are approved. Redacted GCP/Secret Manager metadata and selected GCS location were observed; the bucket class now matches the approved Nearline selection. The required private-boundary absence recheck and controlled Prisma API runner result are unavailable. No provider capability, encryption-at-rest, PostgreSQL compatibility, backup/PITR, restore, or region-to-Singapore proof has been accepted. D11 remains unresolved for the authorized non-production proof of both selected services and approved recovery feasibility. Passing D11 can make P0-08 decision/architecture-ready only; production financial posting, production access, and real-School-data use remain unauthorized pending `P5-12`.

**Exact next action:** System Admin must provide an attributable result from the approved controlled non-exposing Prisma API runner, including job/run reference, redacted HTTP status, matching project ID, and reported region, plus an official provider mapping of `ap-southeast-1` to the selected Singapore location and a redacted bucket-IAM check showing no `allUsers` binding. Then resume `P0-08-D11-01` and run the remaining approved redacted non-production proof.
