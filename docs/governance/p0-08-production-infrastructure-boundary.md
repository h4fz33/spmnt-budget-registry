...
**Status:** Draft approval pack; sponsoring-authority and Product Owner/accountable-reviewer approval required
**Prepared:** 2026-08-10
**Pilot scope:** Shared SchoolBanchee service for all 17 SESAO Narathiwat Schools, with each School remaining an independent financial-record and authorization boundary
...
| Hosting/operator/provider | No production PostgreSQL provider, service tier, region, network boundary, data-residency rule, infrastructure operator, database owner, or support contract is selected in repository evidence. | `OPEN` |
...
| Production | One authoritative shared PostgreSQL write model, private from direct end-user access, reached only by authorized application/runtime and operational paths. Production data and secrets never flow down to development/test. | Logical model is `APPROVED ARCHITECTURE`; provider, operator, topology, region, and service tier are `OPEN` |
...
**Proposed hosting model:** use a managed PostgreSQL service with provider-operated storage durability, automated backups/point-in-time recovery, encryption, monitoring, patching, and documented recovery procedures. This is a candidate because it reduces the pilot's database-operations burden; it is not approved, and no cloud provider or KMS is selected.
...
| --- | --- | --- |
| Recovery Point Objective (RPO) | The Blueprint states a pilot target of `RPO <= 24 hours` unless the sponsoring authority sets a stricter value. | `OPEN`; candidate only, not an approved loss tolerance or provider guarantee |
| Recovery Time Objective (RTO) | The Blueprint states a pilot target of `RTO <= 4 hours` unless the sponsoring authority sets a stricter value. | `OPEN`; candidate only, not an approved restoration commitment or provider guarantee |
...
P0-04 application roles do not automatically confer infrastructure authority. In particular, System Admin's diagnostic/technical-executor boundary does not silently make that role the database owner, backup custodian, secret owner, or encryption-key owner. The sponsoring authority and Product Owner must name the organizational data owner, infrastructure operator, security/key custodian, backup operator, and restore approver, including segregation and emergency access rules.
...
| Retention | P0-07-D10 requires approval of backup generations/period, holds, destruction, access owner, and restore environment. | `OPEN` under BLK-006; P0-08 does not invent a period |
...
| `P0-08-D09` | Jointly approve backup retention/generations, legal-hold interaction, immutability/deletion protection, and secure destruction with P0-07-D10. | `OPEN` under BLK-006; no destructive expiry |
...
| --- | --- |
| `A` | OBEC sponsoring/accountable infrastructure authority, or a delegate whose decision authority and scope are documented. Owns institutional accountability, delegation, production-risk acceptance, and authorization to use the selected production boundary. |
| `P` | SESAO Narathiwat Product Owner/accountable reviewer. Owns pilot business behavior, operational business priorities, outage/offline behavior, and business validation of recovery. It does not become database, backup, or key operator merely by approving a product decision. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `P0-08-D01` | `A` | `O` service/contract/feasibility evidence; `S` security and residency review; `P` confirms pilot suitability | Hosting model, provider/service tier, PostgreSQL version policy, region/data residency, availability topology, contracting organization, and explicit approval/rejection | `OPEN` | `OPEN` | `OPEN` |
...
| `P0-08-D09` | `R` jointly with `A` | `S` approves deletion/immutability protection; `O` confirms implementability and propagation | Backup generations/period, legal-hold behavior, immutability/deletion protection, secure destruction, disposal evidence, and reconciliation with P0-07-D10 | `OPEN` | `OPEN` | `OPEN` |
...
| --- | --- | --- | --- | --- |
| `A`: OBEC sponsoring/accountable infrastructure authority or documented delegate | `OPEN` | `OPEN` | `OPEN` | Approve assigned D01, D02, D04, D05, D07-D11 decisions; identify delegation instrument/scope where used; authorize production only after D11 passes |
| `P`: SESAO Narathiwat Product Owner / accountable reviewer | `OPEN` | `OPEN` | `OPEN` | Approve D03; accept the business inputs/validation assigned under D01, D04, D05, D08, D10, and D11 |
...
| `R`: P0-07 records/retention authority | `OPEN` | `OPEN` | `OPEN` | Approve D09 jointly through P0-07-D10 without changing P0-07 in this task |

## Acceptance Audit

| P0-08 acceptance element | Result | Evidence or gap |
| --- | --- | --- |
| PostgreSQL hosting model, environment separation, operator, and ownership documented | Satisfied as an approval pack | Approved logical model and environment boundaries are explicit; provider/operator/owner remain P0-08-D01/D02 |
