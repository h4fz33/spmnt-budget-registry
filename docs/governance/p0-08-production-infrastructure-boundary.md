...
| Operational service | Prisma Postgres Free, the D01 selected database service, is the intended PostgreSQL backup/PITR service under D08. Google Cloud Storage remains selected only for unconfigured/disabled `OBJECT-PRIVATE`. | `SELECTED INTENT; CAPABILITY GAP OBSERVED; ALTERNATIVE DECISION OPEN`; Prisma's current backup documentation lists automatic daily snapshots only for Starter, Pro, and Business, omitting Free. No provider-managed automated backup/PITR, schedule, monitoring, alerting, secondary copy, restore guarantee, or SLA is accepted |
...
| `P0-08-D08` | `A` authorizes the service boundary | `O` selects and accepts the operational service/schedule; `S` approves access/protection; `P` confirms protected business content | Backup/PITR service, transaction-consistency method, schedule/timezone, monitoring/alerting, failure escalation, secondary-copy strategy, protected content, and access boundary | `PRISMA POSTGRES FREE SELECTED AS INTENDED DATABASE BACKUP/PITR SERVICE; CAPABILITY GAP OBSERVED; ALTERNATIVE DECISION OPEN` | Current Product Owner instruction in this Codex task (no personal name recorded); P0-08-D02-D10-01 session handoff; official provider backup documentation retrieved 2026-08-20 (`https://www.prisma.io/docs/postgres/database/backups`). Product Owner separately authorizes a one-time local synthetic `pg_dump`/`pg_restore` P1-12 proof; it does not alter the provider selection, schedule, or production boundary. | 2026-08-20 |
...
| `P0-08-D08` | Current Product Owner instruction selects Prisma Postgres Free, the D01 database service, as the intended PostgreSQL backup/PITR service. | Prisma's official backup documentation retrieved 2026-08-20 lists automatic daily snapshots only for Starter, Pro, and Business plans; Free is omitted. The same page documents manual `pg_dump`/`pg_restore` with a direct connection and says finer-grained point-in-time restore is future functionality. The Product Owner authorized P1-12 only as a one-time local synthetic dump/restore proof; it neither changes the selected plan nor provides provider-managed capability. | `SELECTED INTENT; CAPABILITY GAP OBSERVED; ALTERNATIVE DECISION OPEN` |
...
| Schema-integrity restore of the implemented application | `P1-12` has a completed test-only local synthetic restore proof. Selected-provider recovery, target achievement, and business acceptance remain `P5-05` work. |
...
| Backup content, frequency, retention, encryption, ownership, restore, verification, and protection documented | Partially satisfied | P1-12 now proves a one-time local synthetic `pg_dump`/`pg_restore` with a clean schema-integrity comparison. D08 still preserves the intended Prisma Postgres Free selection, while provider documentation omits Free from automatic snapshot plans and describes point-in-time restore as future functionality. There is no approved operational schedule, protected destination, retention/key control, selected-provider recovery proof, or RPO/RTO drill. D09 remains policy-in-principle and D10 remains an unexecuted quarterly-drill duty split. |
