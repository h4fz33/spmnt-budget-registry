...
- The P0-04 matrix must assign the Policy Publisher capability to the current Matrix-designated eligible Internal Audit holder and record that no independent reviewer is required before policy activation. Current-status verification, separately approved application scope, and the approved designation remain required for P0-04 closure; remaining authorization values are recorded in the BLK-003 reconciliation record rather than inferred here.
