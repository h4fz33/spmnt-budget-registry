...
- Forms cannot create a Substitute Director Authority, generic role, or permission. Product Owner Decision 13 resolves the limited P0-04 `AUTH-14`/`AUTH-15` model: Acting uses the approved structured reason allowlist and Temporary uses an Active/Acting-unavailability basis; optional uploads are not external appointment prerequisites. These application records do not create governmental appointments.
