...
- [ ] [TODO] `P0-08` Decide MongoDB hosting/topology, connectivity/offline boundary, RPO, RTO, encryption/key ownership, and operational backup service. Dependencies: `P0-01`. MongoDB/Mongoose and NextAuth are selected. Done when the remaining production boundary and recovery targets are documented and transaction support is confirmed.
...
- [ ] [TODO] `P1-03` Establish MongoDB replica-set development/test runtimes, Mongoose connection/configuration, versioned index migrations, local seed/reset workflow, and CI database. Dependencies: `P1-01`, `P1-02`. Done when startup rejects transaction-incapable MongoDB and a clean database can be indexed, seeded, and integration-tested repeatably.
- [ ] [TODO] `P1-04` Implement School/ESAO Organization, membership, role, Fiscal Year, and identifier Mongoose models with duplicated scope, validators, and compound indexes. Dependencies: `P1-03`. Done when duplicate SMIS/MOE codes, cross-school references, and invalid fiscal dates fail under tests/validators.
...
- [ ] [TODO] `P1-09` Implement MongoDB-backed idempotency storage, transactional outbox, and background-job retry conventions. Dependencies: `P1-03`. Done when duplicate command submissions return one result and transaction/job retries create no duplicate business records.
- [ ] [TODO] `P1-10` Implement Decimal128/exact-decimal money, Buddhist/Gregorian fiscal-date, stable reference, and atomic scoped numbering primitives. Dependencies: `P1-01`, `P1-03`. Done when parsing, rounding rejection, serialization, year rollover, transaction retry, gap, and duplicate tests pass.
...
- [ ] [TODO] `P1-12` Establish automated MongoDB backup and a documented restore/index-integrity smoke test. Dependencies: `P1-03`, decision `P0-08`. Done when a backup is restored into a clean replica-set environment and documents, indexes, references, and totals verify.
- [ ] [TODO] `P1-13` Establish CI checks for formatting, lint, type checks, unit tests, MongoDB replica-set integration tests, index migrations, seeds, and production build. Dependencies: `P1-01`, `P1-03`. Done when protected checks run deterministically from a clean checkout.
...
- [ ] [TODO] `P1-GATE` Approve Phase 1. Dependencies: `P1-01` through `P1-17`. Done when registration/approval, authenticated school isolation, policy resolution, audit logging, idempotency, MongoDB transaction/index migration, build, and restore evidence pass.
...
- [ ] Mongoose schemas, MongoDB validators/indexes, reference integrity, and migration/rollback behavior are reviewed.
