-- P1-16: a Substitute Director Authority is immutable except for a revisioned
-- lifecycle transition. The P1-04 shared trigger originally protected the
-- recordVersion itself, which made every legitimate transition fail.
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'SubstituteDirectorAuthority'
    AND (
      (new_record -> 'status' IS DISTINCT FROM old_record -> 'status'
        OR new_record -> 'recordVersion' IS DISTINCT FROM old_record -> 'recordVersion')
      AND (
        (new_record ->> 'recordVersion')::INTEGER <> (old_record ->> 'recordVersion')::INTEGER + 1
        OR new_record -> 'status' IS NOT DISTINCT FROM old_record -> 'status'
      )
    ) THEN
